...
/**
 * Implements Drag and Drop for a JTabbedPane Element
 *
 * @author Bryan Beck
 * @since 04.09.2012
 */
public class MyDnDJTabbedPane extends JTabbedPane {
    ...
    private void autoScrollTest(Point glassPt) {
        ...
        int tabPlacementLocal = getTabPlacement();
        if (tabPlacementLocal == TOP || tabPlacementLocal == BOTTOM) {
            rBackward.setBounds(r.x, r.y, rwh, r.height);
            rForward.setBounds(
                    r.x + r.width - rwh - buttonsize, r.y, rwh + buttonsize, r.height);
        } else if (tabPlacementLocal == LEFT || tabPlacementLocal == RIGHT) {
    ...
    public MyDnDJTabbedPane() {
        ...
        final DragGestureListener dgl = new DragGestureListener() {
            ...
            public void dragGestureRecognized(DragGestureEvent e) {
                ...
                //MODIFIED
                Component dragTab = getComponentAt(dragTabIndex);
                if((dragTab instanceof AbstractTabGUI) && ((AbstractTabGUI)dragTab).isSearchTab()){
                    return;
                }
                //END MODIFICATION
                ...
                try {
                    ...
                    System.err.println(idoe.toString());
...
//            System.out.println("dragExit");
    ...
    private int getTargetTabIndex(Point glassPt) {
        Point tabPt = SwingUtilities.convertPoint(
                glassPane, glassPt, MyDnDJTabbedPane.this);
    ...
    private void convertTab(int prev, int next) {
        ...
        //MODIFIED
        Component nxt = getComponentAt(tgtindex);
        //Letzter Tab oder Search Tab
        if(nxt == null || 
                ((nxt instanceof AbstractTabGUI) && ((AbstractTabGUI)nxt).isSearchTab())){
            return;      
        }           
        //END MODIFICATION
    ...
    private void initTargetLeftRightLine(int next) {      
        ...
        int tgtindex = dragTabIndex > next ? next : next - 1;
        Component nxt = getComponentAt(tgtindex);
        //Letzter Tab oder Search Tab
        if(nxt == null || 
                ((nxt instanceof AbstractTabGUI) && ((AbstractTabGUI)nxt).isSearchTab())){
    ...
    private Rectangle getTabAreaBounds() {
        ...
        int tabPlacementLocal = getTabPlacement();
        if (tabPlacementLocal == TOP) {
            tabbedRect.height = tabbedRect.height - compRect.height;
        } else if (tabPlacementLocal == BOTTOM) {
            tabbedRect.y = tabbedRect.y + compRect.y + compRect.height;
            tabbedRect.height = tabbedRect.height - compRect.height;
        } else if (tabPlacementLocal == LEFT) {
            tabbedRect.width = tabbedRect.width - compRect.width;
        } else if (tabPlacementLocal == RIGHT) {
